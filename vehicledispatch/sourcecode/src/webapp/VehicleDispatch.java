...
@WebServlet(name = "VehicleDispatch")
public class VehicleDispatch extends HttpServlet {
    ...
    String printVehicleDispatched ( int dist[], int unsorteddist[], int n, int src, int typ){
        ...
        for (int i = 0; i < n; i++) {
            ...
            sourcenode = zipConversion1(item1);
            int type1[];
            type1 = Vehicleavailable.get(item1);
            inner:
            if (type1[typ] > 0) {
                value3= "vehicle dispatched from destination node within this time frame"  +"\t" + dist[i] + "mins" + "\t"+ "and from node" +sourcenode;
                type1[typ]--;
                break  Label1;
